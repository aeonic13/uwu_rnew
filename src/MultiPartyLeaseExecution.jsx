...
const buildSignatories = groupApplication => {
  ...
  groupApplication.members.forEach(m => {
    list.push({
      ...
      name: m.name,
      email:
        m.email || `${m.name.toLowerCase().replace(' ', '.')}@university.edu`,
      signed: false,
      signedAt: null,
    })
...
const TenantLeaseScreen = ({
  ...
  const mySignatory = signatories.find(
    s => s.role === 'tenant' && s.name === (currentUser?.name || 'You')
  )
      ...
      <div className="bg-brand-50 border border-brand-200 rounded-xl p-4">
        ...
        <div className="flex flex-wrap gap-3 text-sm text-brand-600 mt-2">
          ...
          <span className="flex items-center">
            ...
            {groupApplication?.members?.length || 1} tenant
            {(groupApplication?.members?.length || 1) !== 1 ? 's' : ''}
          </span>
          <span className="flex items-center">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4 space-y-3">
        ...
        <div className="text-sm text-gray-600 space-y-2">
          <p>
            ...
            Guarantors co-sign and are legally responsible if their student
            defaults.
          </p>
        ...
        <>
          <label className="flex items-start space-x-3 cursor-pointer">
            ...
            <span className="text-sm text-gray-700">
              I have read and agree to the joint-and-several lease terms,
              including all clauses regarding rent responsibility, security
              deposit, and early termination.
            </span>
          </label>
...
const GuarantorLeaseScreen = ({
  ...
  const guarantorSig = signatories.find(
    s => s.role === 'guarantor' && !s.signed
  )
    ...
    <div className="p-4 pb-20 space-y-6">
      <div className="bg-purple-50 border border-purple-200 rounded-xl p-4">
        <p className="font-semibold text-purple-800 mb-1">
          Guarantor Co-Signature
        </p>
        <p className="text-sm text-purple-700">
          Review and co-sign the lease for{' '}
          <strong>{guarantorSig.forTenant}</strong> and roommates at{' '}
      ...
      <label className="flex items-start space-x-3 cursor-pointer">
        ...
        />
        <span className="text-sm text-gray-700">
          I have read and agree to the guarantor terms and understand my
          obligations.
        </span>
      </label>
  ...
  <div className="p-4 pb-20 space-y-6 text-center">
    <div className="space-y-2 pt-4">
      ...
      <p className="text-gray-600 text-sm">
        The group is officially locked in. Your lease has been saved to your
        Rentra Document Vault.
      </p>
    </div>
          ...
          <div key={i} className="flex items-center text-sm text-green-700">
            ...
            {m.guarantor && (
              <>
                <Shield
                  size={13}
                  className="ml-3 mr-1 text-purple-500 flex-shrink-0"
                />
...
const MultiPartyLeaseExecution = ({
  ...
  const [signatories, setSignatories] = useState(() =>
    buildSignatories(groupApplication)
  )
  ...
  const markSigned = (signatoryId, signature) => {
    setSignatories(prev => {
      ...
      if (allSigned) {
        ...
        if (
          allTenantsSigned &&
          updated.some(s => s.role === 'guarantor' && !s.signed)
        ) {
