...
import { ArrowLeft, Building2, Wrench, Droplets, Zap, Flame, AlertTriangle, Camera, Upload } from 'lucide-react';
...
const ReportMaintenanceView = ({ user, onBack }) => {
  ...
  const maintenanceTypes = [
    ...
    { value: 'structural', label: 'Structural/Walls', icon: Building2, color: 'text-gray-800' },
    { value: 'pest', label: 'Pest Control', icon: AlertTriangle, color: 'text-orange-600' },
    { value: 'general', label: 'General Repair', icon: Wrench, color: 'text-green-600' },
    { value: 'other', label: 'Other', icon: Wrench, color: 'text-purple-600' }
  ];

  const priorities = [
    { value: 'low', label: 'Low - Routine maintenance', color: 'text-green-600' },
    { value: 'medium', label: 'Medium - Needs attention soon', color: 'text-yellow-600' },
    { value: 'high', label: 'High - Important but not urgent', color: 'text-orange-600' },
    { value: 'emergency', label: 'Emergency - Immediate attention required', color: 'text-red-600' }
  ];
  ...
  const handleSubmit = () => {
    ...
    alert('Maintenance request submitted successfully! Your landlord will be notified and will respond within 24-48 hours.');
    onBack();
  };
  ...
  return (
    <div className="p-4 pb-20">
      ...
      <div className="space-y-6">
        {/* Maintenance Type */}
        <div>
          <label className="block text-sm font-medium mb-3">What type of maintenance is needed?</label>
        ...
        {/* Location */}
        <div>
          <label className="block text-sm font-medium mb-1">Location in property</label>
          <input
            type="text"
            value={location}
        ...
        <div>
          ...
          <p className="text-xs text-gray-500 mb-3">
            Upload photos to help your landlord understand the issue better. Max 5 photos.
          </p>
        ...
        {priority === 'emergency' && (
          <div className="bg-red-50 p-4 rounded-lg border border-red-200">
            <div className="flex items-center mb-2">
              ...
              <h4 className="font-semibold text-red-800">Emergency Maintenance</h4>
            </div>
            <p className="text-sm text-red-700">
              For true emergencies that pose immediate danger (gas leaks, electrical hazards, flooding), 
              contact emergency services (911) first, then your landlord directly by phone.
            </p>
          </div>
        )}
