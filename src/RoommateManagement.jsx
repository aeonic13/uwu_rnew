...
import { Users, Plus, Mail, Phone, DollarSign, Calendar, Home, Settings, MessageCircle, AlertTriangle, CheckCircle, Clock, UserPlus, UserMinus, Shield, Star, CreditCard, FileText, Bell, X, ArrowRight } from 'lucide-react';

const RoommateManagement = ({ property, currentUser, userType, onBack, onNavigateToSecurityDeposit }) => {
  ...
  const [rentalData, setRentalData] = useState({
    ...
    financials: {
      ...
      utilityBills: [
        { type: 'electricity', amount: 180, dueDate: '2024-03-20', status: 'pending' },
  ...
  const occupancyRate = Math.round((activeTenants.length / rentalData.tenants.length) * 100);
        ...
        <div className="bg-gray-50 rounded-lg p-3">
          ...
          <div className="flex items-center justify-between mt-1">
            <span className="text-sm text-gray-600">
              {rentalData.property.bedrooms} bed • {rentalData.property.bathrooms} bath
            </span>
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            ...
            <div className="bg-yellow-50 p-4 rounded-lg border border-yellow-200">
              <div className="text-2xl font-bold text-yellow-600">{vacantRooms.length}</div>
              <div className="text-sm text-yellow-700">Vacant Rooms</div>
            </div>
            <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
              <div className="text-2xl font-bold text-purple-600">{rentalData.pendingInvitations.length}</div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="space-y-3">
              ...
              <div className="flex items-center">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium">Lease agreement signed by Alex Johnson</p>
              ...
              >
                <div className="flex items-center">
                  ...
                  <div className="text-left">
                    <p className="font-medium">{userType === 'owner' ? 'Manage Security Deposits' : 'Security Deposit Protection'}</p>
                    <p className="text-sm text-gray-600">View deposit status and protection details</p>
                  </div>
                </div>
          ...
          {/* Pending Actions */}
          {(rentalData.pendingInvitations.length > 0 || vacantRooms.length > 0) && (
              ...
              <div className="space-y-2">
                {rentalData.pendingInvitations.length > 0 && (
                  <p className="text-sm text-yellow-700">
                    • {rentalData.pendingInvitations.length} pending roommate invitation(s)
                  </p>
                )}
                {vacantRooms.length > 0 && (
                  <p className="text-sm text-yellow-700">
                    • {vacantRooms.length} vacant room(s) available for new tenants
                  </p>
                )}
              </div>
            </div>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            {rentalData.tenants.map((tenant, index) => (
              <div key={tenant.id} className="bg-white border border-gray-200 rounded-lg p-4 mb-4">
                {tenant.status === 'vacant' ? (
                  <div className="flex items-center justify-between">
                    <div className="flex items-center">
                      ...
                      <div>
                        <h4 className="font-semibold text-gray-600">Room Available</h4>
                    ...
                    <div className="flex items-center flex-1">
                      ...
                      <div className="flex-1">
                        ...
                        <div className="flex items-center mt-2">
                          ...
                          <span className={`ml-3 px-2 py-1 text-xs rounded-full ${getStatusColor(tenant.paymentStatus)}`}>
                            {tenant.paymentStatus === 'current' ? 'Paid' : tenant.paymentStatus}
                          </span>
                        </div>
                      </div>
                    </div>
                  ...
                  <div className="mt-4 pt-4 border-t border-gray-100">
                    <div className="grid grid-cols-2 gap-4 text-sm">
                      ...
                      <div>
                        ...
                        <p className="font-medium capitalize">{tenant.backgroundCheck}</p>
                      </div>
                    </div>
                  </div>
                )}
          ...
          {rentalData.pendingInvitations.length > 0 && (
            <div>
              ...
              {rentalData.pendingInvitations.map((invite) => (
                <div key={invite.id} className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 mb-3">
                  <div className="flex items-center justify-between">
                    <div>
                      <h4 className="font-semibold">{invite.name}</h4>
                      <p className="text-sm text-gray-600">{invite.email}</p>
                      <p className="text-sm text-gray-500">
                        Invited {new Date(invite.invitedDate).toLocaleDateString()} • ${invite.rentAmount}/month
                      </p>
                    </div>
                    <div className="text-right">
                      ...
                      <p className="text-xs text-gray-500 mt-1">
                        Move-in: {new Date(invite.moveInDate).toLocaleDateString()}
                      </p>
                    </div>
                  </div>
                </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="grid grid-cols-1 gap-4">
              <div className="flex justify-between items-center">
                ...
                <span className="font-semibold">${rentalData.financials.totalMonthlyRent}</span>
              </div>
              <div className="flex justify-between items-center">
                <span>Collected This Month:</span>
                <span className="font-semibold text-green-600">${rentalData.financials.collectedThisMonth}</span>
              </div>
              <div className="flex justify-between items-center">
                <span>Outstanding Amount:</span>
                <span className="font-semibold text-red-600">${rentalData.financials.outstandingAmount}</span>
              </div>
              <div className="border-t pt-2 flex justify-between items-center">
                <span>Security Deposits Held:</span>
                <span className="font-semibold">${rentalData.financials.securityDeposits}</span>
              </div>
            </div>
          </div>
            ...
            <div className="space-y-3">
              {activeTenants.map((tenant) => (
                <div key={tenant.id} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  <div className="flex items-center">
                    <img src={tenant.avatar} alt={tenant.name} className="w-8 h-8 rounded-full mr-3" />
                    <div>
                      <p className="font-medium">{tenant.name}</p>
                      <p className="text-sm text-gray-600">${tenant.rentAmount}/month</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <span className={`px-2 py-1 text-xs rounded-full ${getStatusColor(tenant.paymentStatus)}`}>
                      {tenant.paymentStatus === 'current' ? 'Paid' : tenant.paymentStatus}
                    </span>
                    <p className="text-xs text-gray-500 mt-1">
          ...
          <div className="bg-red-50 border border-red-200 rounded-lg p-6">
            ...
            <div className="space-y-3">
              ...
              <p className="text-sm text-red-700">
                This will end all active leases and remove all tenants. This action cannot be undone.
              </p>
            </div>
          </div>
        </div>
          ...
          <div className="bg-white rounded-lg max-w-md w-full">
            <div className="p-6">
              ...
              <p className="text-gray-600 mb-6">
                Are you sure you want to remove this roommate? This will end their lease and they will need to vacate the property.
              </p>
