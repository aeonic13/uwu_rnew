...
import { 
  Calendar, DollarSign, Clock, CheckCircle, XCircle, AlertTriangle,
  Users, CreditCard, Mail, Bell, Settings, TrendingUp, BarChart3,
  ArrowUpRight, ArrowDownRight, Repeat, Zap, Shield, Target,
  Phone, MessageSquare, FileText, Download, Upload, Eye,
...
const RentCollectionSystem = ({ user, onBack, onNavigate }) => {
  ...
  return (
    <div className="p-4 pb-20">
      <div className="mb-6">
        <h2 className="text-2xl font-bold mb-2">Rent Collection & Automation</h2>
      ...
      <div className="flex mb-6 bg-gray-100 rounded-lg p-1 overflow-x-auto">
        {['dashboard', 'tenants', 'automation', 'communications', 'reports'].map((tab) => (
          <button
            key={tab}
        ...
        <div className="space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="bg-green-50 p-4 rounded-lg border border-green-200">
              <div className="flex items-center justify-between mb-2">
                <div className="text-2xl font-bold text-green-600">${rentData.overview.collectedRent.toLocaleString()}</div>
                <ArrowUpRight size={20} className="text-green-600" />
              </div>
              <div className="text-sm text-green-700">Rent Collected</div>
              <div className="text-xs text-green-600 mt-1">{rentData.overview.collectionRate}% collection rate</div>
            </div>
            
            <div className="bg-red-50 p-4 rounded-lg border border-red-200">
              <div className="flex items-center justify-between mb-2">
                <div className="text-2xl font-bold text-red-600">${rentData.overview.overdueRent.toLocaleString()}</div>
            ...
            <div className="bg-purple-50 p-4 rounded-lg border border-purple-200">
              <div className="flex items-center justify-between mb-2">
                <div className="text-2xl font-bold text-purple-600">${rentData.overview.lateFeeCollected}</div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="grid grid-cols-3 gap-4 mb-4">
              <div className="text-center">
                <div className="text-xl font-bold text-green-600">{rentData.overview.onTimePayments}</div>
                <div className="text-sm text-gray-600">On Time</div>
              </div>
              <div className="text-center">
                <div className="text-xl font-bold text-yellow-600">{rentData.overview.latePayments}</div>
                <div className="text-sm text-gray-600">Late</div>
              </div>
              <div className="text-center">
                <div className="text-xl font-bold text-red-600">{rentData.overview.missedPayments}</div>
                <div className="text-sm text-gray-600">Missed</div>
              </div>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-3">
              <div className="bg-green-500 h-3 rounded-l-full" style={{ width: `${(rentData.overview.onTimePayments / rentData.overview.totalTenants) * 100}%` }}></div>
            </div>
          </div>
              ...
              <button className="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                <div className="flex items-center">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-sm text-gray-600">1 tenant overdue by 18 days</p>
                  </div>
                </div>
                <ArrowRight size={16} className="text-gray-400" />
              </button>
              ...
              <button className="flex items-center justify-between p-3 border border-gray-200 rounded-lg hover:bg-gray-50 transition-colors">
                <div className="flex items-center">
                  ...
                  <div className="text-left">
                    ...
                    <p className="text-sm text-gray-600">8 tenants not enrolled</p>
                  </div>
                </div>
                <ArrowRight size={16} className="text-gray-400" />
              </button>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="space-y-3">
              {rentData.tenants.filter(t => t.status === 'paid').slice(0, 3).map((tenant) => (
                <div key={tenant.id} className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                  <div className="flex items-center">
                    <img src={tenant.avatar} alt={tenant.name} className="w-10 h-10 rounded-full mr-3" />
                    <div>
                  ...
                  <div className="text-right">
                    <p className="font-semibold text-green-600">${tenant.rentAmount.toLocaleString()}</p>
                    <p className="text-xs text-gray-500">{tenant.paymentMethod} • {tenant.paidDate}</p>
                  </div>
                </div>
              ))}
            </div>
          ...
          {rentData.tenants.map((tenant) => (
            <div key={tenant.id} className="bg-white border border-gray-200 rounded-lg p-6">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-center">
                  <img src={tenant.avatar} alt={tenant.name} className="w-12 h-12 rounded-full mr-4" />
                  <div>
                    <div className="flex items-center">
                ...
                <div className="text-right">
                  <div className="flex items-center mb-1">
                    ...
                    <span className={`ml-2 px-3 py-1 text-sm rounded-full ${getStatusColor(tenant.status)}`}>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4">
                <div>
                  ...
                  <div className="font-semibold">${tenant.rentAmount.toLocaleString()}</div>
                </div>
                <div>
                  <div className="text-sm text-gray-600">Total Owed</div>
                  <div className={`font-semibold ${tenant.totalOwed > 0 ? 'text-red-600' : 'text-green-600'}`}>
                    ${tenant.totalOwed.toLocaleString()}
                  </div>
                </div>
                <div>
                  <div className="text-sm text-gray-600">Payment Method</div>
                  <div className="font-medium">{tenant.paymentMethod.toUpperCase()}</div>
                </div>
                <div>
                  <div className="text-sm text-gray-600">Due Date</div>
                  <div className="font-medium">{new Date(tenant.dueDate).toLocaleDateString()}</div>
                </div>
              </div>
      ...
      {activeTab === 'automation' && (
        <div className="space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              ...
              <div>
                ...
                <div className="text-xl font-bold text-green-600">{rentData.automation.autoPayments.successRate}%</div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="space-y-4">
              {Object.entries(rentData.automation.reminderSettings).map(([key, setting]) => (
                  ...
                  <div className="flex items-center">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">
                        {setting.daysBefore ? `${setting.daysBefore} days before` : `${setting.daysAfter} days after`} due date
                      </p>
                    </div>
                  </div>
                  <div className="text-sm text-gray-600">
                    {setting.method === 'both' ? 'Email + SMS' : setting.method.replace('_', ' ').toUpperCase()}
                  </div>
                </div>
              ))}
            </div>
            ...
            <div key={comm.id} className="bg-white border border-gray-200 rounded-lg p-4">
              <div className="flex items-center justify-between mb-3">
                <div className="flex items-center">
                  <div className={`w-3 h-3 rounded-full mr-3 ${
                    comm.type === 'overdue_notice' ? 'bg-red-500' :
                    comm.type === 'reminder' ? 'bg-yellow-500' :
                    'bg-green-500'
                  }`}></div>
                  <div>
                ...
                <div className="text-right">
                  <div className="flex items-center text-sm text-gray-500">
                    {comm.method === 'email' && <Mail size={16} className="mr-1" />}
                    {comm.method === 'sms' && <MessageSquare size={16} className="mr-1" />}
                    {comm.method === 'both' && (
                      <>
              ...
              <div className="flex items-center justify-between pt-3 border-t border-gray-200">
                <span className={`px-3 py-1 text-sm rounded-full ${
                  comm.status === 'delivered' ? 'bg-green-100 text-green-800' : 'bg-yellow-100 text-yellow-800'
                }`}>
                  {comm.status}
                </span>
                <div className="flex space-x-2">
                ...
                <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="flex items-center space-x-4">
                    <div className="text-sm">
                      <span className="text-gray-600">${month.collected.toLocaleString()}</span>
                      <span className="text-gray-400 mx-1">/</span>
                      <span className="text-gray-500">${month.expected.toLocaleString()}</span>
                    </div>
                    <div className={`font-semibold ${
                      month.rate >= 95 ? 'text-green-600' : 
                      month.rate >= 90 ? 'text-yellow-600' : 'text-red-600'
                    }`}>
                      {month.rate}%
                    </div>
                  </div>
            ...
            <div className="space-y-3">
              {rentData.reports.paymentMethods.map((method, index) => (
                <div key={index} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                  ...
                  <div className="text-right">
                    <div className="font-semibold">{method.count} tenants ({method.percentage}%)</div>
                    <div className="text-sm text-gray-600">Avg: {method.avgTime} days to clear</div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                <span className="font-semibold text-green-600">{rentData.overview.collectionRate}%</span>
              </div>
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
              ...
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                ...
                <span className="font-semibold text-purple-600">{rentData.automation.autoPayments.enrollmentRate}%</span>
              </div>
              <div className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                <span>Late Fee Collection</span>
                <span className="font-semibold text-orange-600">${rentData.overview.lateFeeCollected}</span>
              </div>
            </div>
          </div>
        </div>
