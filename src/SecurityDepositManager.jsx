...
const SecurityDepositManager = ({ lease, userType, onBack }) => {
  ...
  const handleCalculateReturn = () => {
    const totalDeductions = depositData.deductions.reduce(
      (sum, deduction) =>
        deduction.status === 'approved' ? sum + deduction.amount : sum,
      0
    )
  ...
  const daysUntilReturn = Math.ceil(
    (new Date(depositData.deposit.returnDueDate) - new Date()) /
      (1000 * 60 * 60 * 24)
  )
      ...
      <div className="mb-6">
        ...
        <div className="bg-brand-50 border border-brand-200 rounded-lg p-3">
          ...
          <p className="text-sm text-brand-500 mt-1">
            ${depositData.deposit.originalAmount} held securely • California
            law: 21-day return
          </p>
        </div>
      </div>
        ...
        <div className="space-y-6">
          ...
          <div className="bg-white border border-gray-200 rounded-lg p-6">
            <div className="flex items-center justify-between mb-4">
              ...
              <span
                className={`px-3 py-1 rounded-full text-sm font-medium ${getStatusColor(depositData.deposit.status)}`}
              >
                {depositData.deposit.status.charAt(0).toUpperCase() +
                  depositData.deposit.status.slice(1)}
              </span>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 mb-4">
              <div className="text-center">
                <div className="text-2xl font-bold text-green-600">
                  ${depositData.deposit.originalAmount}
                </div>
              ...
              <div className="flex items-center justify-between p-3 bg-green-50 rounded-lg">
                <div className="flex items-center">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-600">
                      Completed{' '}
                      {new Date(
                        depositData.moveInInspection.date
                      ).toLocaleDateString()}
                    </p>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-sm font-medium">
                    {depositData.moveInInspection.issuesFound} issues
                  </div>
                  <div className="text-xs text-gray-500">
                    {depositData.moveInInspection.photosUploaded} photos
                  </div>
                </div>
              </div>
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-4">
            ...
            <ul className="space-y-2 text-sm text-green-700">
              ...
              <li>
                • Legal compliance with {depositData.deposit.state} housing laws
              </li>
            </ul>
          </div>
        </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-600 mb-3">
                    Upload receipts, invoices, or photos as evidence for this
                    deduction
                  </p>
                        ...
                        {deductionForm.receiptFiles.length > 0 && (
                          <div className="mb-3">
                            ...
                            <div className="space-y-2">
                              {deductionForm.receiptFiles.map(
                                (fileObj, index) => (
                                  <div
                                    key={index}
                                    ...
                                    <div className="flex items-center">
                                      ...
                                      <div>
                                        ...
                                        <p className="text-xs text-gray-500">
                                          {(fileObj.size / 1024 / 1024).toFixed(
                                            2
                                          )}{' '}
                                          MB
                                        </p>
              ...
              <div className="space-y-4">
                ...
                <div className="bg-brand-50 border border-brand-200 rounded-lg p-3">
                  <p className="text-sm text-brand-600">
                    <strong>State Requirement:</strong> Deposit must be returned
                    within {depositData.deposit.returnDeadline} days of lease
                    end in {depositData.deposit.state}.
                  </p>
                </div>
