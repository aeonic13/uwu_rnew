  ...
  members.flatMap(member => {
    const items = [
      {
        ...
        status: member.applicationStatus === 'complete' ? 'verified' : 'pending',
      },
      {
        id: `app-${member.name}`,
        memberName: member.name,
    ...
    if (member.guarantor) {
      items.push({
        ...
        status: member.guarantor.verificationStatus === 'verified'
          ? 'verified'
...
const statusIcon = status => {
  if (status === 'verified') return <CheckCircle size={18} className="text-green-500 flex-shrink-0" />
  if (status === 'pending')  return <Clock size={18} className="text-yellow-500 flex-shrink-0" />
  return <XCircle size={18} className="text-red-400 flex-shrink-0" />
}

const statusLabel = status => {
  if (status === 'verified')    return { text: 'Verified',     cls: 'bg-green-100 text-green-700' }
  if (status === 'pending')     return { text: 'In Progress',  cls: 'bg-yellow-100 text-yellow-700' }
      ...
      {PIPELINE_STAGES.map((stage, i) => (
        <React.Fragment key={stage.key}>
          ...
          {i < PIPELINE_STAGES.length - 1 && (
            <div className={`flex-1 h-0.5 mx-1 mt-[-14px] ${ i < idx ? 'bg-green-400' : 'bg-gray-200' }`} />
          )}
        </React.Fragment>
      ))}
    </div>
...
const ApplicationWaitingRoom = ({
  ...
  // Effective status
  const effectiveStatus = allVerified && appData.status === 'pending_verifications'
    ? 'ready_for_review'
    : appData.status
      ...
      setAppData(prev => ({
        ...
        members: prev.members.map(m => {
          if (`guar-${m.name}` === itemId && m.guarantor) {
            return { ...m, guarantor: { ...m.guarantor, verificationStatus: 'pending' } }
          }
  ...
  const handleSimulateProgress = () => {
    ...
    setAppData(prev => ({
      ...prev,
      members: prev.members.map(m => {
        if (firstPending.type === 'guarantor' && firstPending.forTenant === m.name && m.guarantor) {
          return { ...m, guarantor: { ...m.guarantor, verificationStatus: 'verified' } }
        }
        if (
  ...
  const combinedIncome = appData.members?.reduce((s, m) => s + (m.monthlyIncome || 0), 0) || 0
    ...
    <div className="p-4 pb-24 space-y-5">
      ...
      <div className="bg-purple-50 border border-purple-200 rounded-xl p-4">
        <div className="flex items-center mb-1">
          ...
          <span className="font-semibold text-purple-800">{appData.groupName}</span>
        </div>
        ...
        <div className="flex gap-3 mt-2 text-xs text-purple-600">
          <span>{totalMembers} tenant{totalMembers !== 1 ? 's' : ''}</span>
          {totalGuarantors > 0 && <span>· {totalGuarantors} guarantor{totalGuarantors !== 1 ? 's' : ''}</span>}
        </div>
      </div>

      {/* Pipeline stepper */}
      <div className="bg-white border border-gray-200 rounded-xl p-4">
        <p className="text-xs font-semibold text-gray-500 uppercase tracking-wide mb-3">Application Pipeline</p>
      ...
      {allVerified ? (
        <div className="bg-green-50 border border-green-200 rounded-xl p-4 flex items-start">
          ...
          <div>
            <p className="font-semibold text-green-800">All verifications complete!</p>
            <p className="text-sm text-green-700 mt-0.5">This application is ready for your review and approval.</p>
          </div>
        </div>
      ) : (
        <div className="bg-yellow-50 border border-yellow-200 rounded-xl p-4 flex items-start">
          ...
          <div>
            <p className="font-semibold text-yellow-800">
              {pendingItems.length} verification{pendingItems.length !== 1 ? 's' : ''} pending
            </p>
            <p className="text-sm text-yellow-700 mt-0.5">
              The system sends automatic reminders. You can also nudge individuals below.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4">
        ...
        <div className="grid grid-cols-3 gap-2 text-center">
          ...
          <div>
            <p className="text-lg font-bold text-gray-700">${incomeThreshold.toLocaleString()}</p>
          ...
          return (
            <div key={mi} className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              ...
              <div className="flex items-center px-4 py-3 bg-gray-50 border-b border-gray-100">
                ...
                <span className="text-xs text-gray-500">${(member.monthlyIncome || 0).toLocaleString()}/mo</span>
              </div>
      ...
      {!allVerified && (
        <p className="text-center text-xs text-gray-400">
          Once all {checklist.length} items are verified, you can review and approve.
        </p>
      )}
    </div>
  )
