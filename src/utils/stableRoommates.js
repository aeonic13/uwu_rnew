...
export class StableRoommatesAlgorithm {
  ...
  generatePreferences() {
    this.students.forEach(student => {
      ...
      const otherStudents = this.students.filter(
        other => other.id !== student.id
      )
      ...
      this.preferences[student.id] = compatibilityScores.map(
  ...
  proposalPhase() {
    ...
    while (freeQueue.length > 0) {
      ...
      const nextTarget = this.reducedLists[personId].find(
        targetId => !proposalsMade[personId].has(targetId)
      )
      ...
      if (!this.proposals[nextTarget]) {
        ...
        if (
          this.ranks[nextTarget][personId] <
          this.ranks[nextTarget][currentProposer]
        ) {
  ...
  firstReduction() {
    Object.keys(this.proposals).forEach(personId => {
      ...
      this.reducedLists[personId] = this.reducedLists[personId].filter(
        otherId => {
          if (this.ranks[personId][otherId] > proposerRank) {
            // Mutual removal
            this.reducedLists[otherId] = this.reducedLists[otherId].filter(
              id => id !== personId
            )
            return false
          }
  ...
  findCycleFromNode(nodeId, visited, path, currentPath) {
    ...
    if (topChoice) {
      const cycle = this.findCycleFromNode(topChoice, visited, path, [
        ...currentPath,
      ])
      if (cycle.length > 0) return cycle
    }
  ...
  eliminateCycle(cycle) {
    for (let i = 0; i < cycle.length; i++) {
      ...
      this.reducedLists[previous] = this.reducedLists[previous].filter(
        id => id !== current
      )
      this.reducedLists[current] = this.reducedLists[current].filter(
        id => id !== previous
      )
    }
  }
  ...
  handleNewProposals() {
    ...
    needsNewProposal.forEach(personId => {
      ...
      if (!this.proposals[topChoice]) {
        ...
        if (
          this.ranks[topChoice][personId] <
          this.ranks[topChoice][currentProposer]
        ) {
          this.proposals[topChoice] = personId
        }
      }
  ...
  calculateCompatibilityScore(student1, student2) {
    ...
    if (student1.budget && student2.budget) {
      const overlap =
        Math.min(student1.budget.max, student2.budget.max) -
        Math.max(student1.budget.min, student2.budget.min)
      if (overlap > 0) {
        const avgRange =
          (student1.budget.max -
            student1.budget.min +
            (student2.budget.max - student2.budget.min)) /
          2
