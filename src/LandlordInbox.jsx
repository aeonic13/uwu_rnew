...
const stageBadge = key => {
  ...
  return <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${cls}`}>{s.label}</span>
}
    ...
    members: [
      {
        ...
        guarantor: { name: 'Jane Kim', email: 'jane@email.com', verificationStatus: 'verified' },
        monthlyIncome: 3200,
      },
      {
        name: 'Mike Torres',
        ...
        university: 'USC',
        applicationStatus: 'complete',
        guarantor: { name: 'Tom Torres', email: 'tom@email.com', verificationStatus: 'verified' },
        monthlyIncome: 2800,
      },
      {
        name: 'David Park',
    ...
    members: [
      {
        ...
        university: 'UCLA',
        applicationStatus: 'complete',
        guarantor: { name: 'Wei Chen', email: 'wei@email.com', verificationStatus: 'verified' },
        monthlyIncome: 4800,
      },
      {
        name: 'Lucas Rivera',
        ...
        guarantor: { name: 'Maria Rivera', email: 'maria@email.com', verificationStatus: 'not_invited' },
        monthlyIncome: 4800,
      },
    ],
  },
    ...
    members: [
      {
        ...
        university: 'USC',
        applicationStatus: 'complete',
        guarantor: { name: 'Derek Johnson', email: 'derek@email.com', verificationStatus: 'verified' },
        monthlyIncome: 5000,
      },
      {
        name: 'Priya Nair',
        ...
        university: 'USC',
        applicationStatus: 'complete',
        guarantor: { name: 'Anand Nair', email: 'anand@email.com', verificationStatus: 'pending' },
        monthlyIncome: 4200,
      },
      {
        name: 'James Wu',
...
const GroupApplicationsTab = ({
  ...
  const guarStatus = v => {
    if (v === 'verified')    return <span className="text-green-600 text-xs font-medium">✅ Verified</span>;
    if (v === 'pending')     return <span className="text-yellow-600 text-xs font-medium">⏳ Pending</span>;
    return                          <span className="text-red-500 text-xs font-medium">❌ Not Invited</span>;
  };
  ...
  if (selectedGroup) {
    ...
    const allVerified = app.members.every(m =>
      !m.guarantor || m.guarantor.verificationStatus === 'verified'
    );
    const allComplete = app.members.every(m => m.applicationStatus === 'complete');
    const readyToApprove = allVerified && allComplete && app.meetsRequirement;
    const alreadyApproved = ['applicants_approved', 'lease_sent', 'pending_signatures', 'fully_executed'].includes(app.status);
      ...
      <div className="p-4 pb-24 space-y-5">
        ...
        <div className="bg-purple-50 border border-purple-200 rounded-xl p-4">
          <div className="flex items-center justify-between mb-1">
            <div className="flex items-center">
              ...
              <span className="font-semibold text-purple-800">{app.groupName}</span>
            </div>
          ...
          <div className="flex gap-3 mt-2 text-xs text-purple-600">
            <span>{app.members.length} tenant{app.members.length !== 1 ? 's' : ''}</span>
            {guarantorCount > 0 && <span>· {guarantorCount} guarantor{guarantorCount !== 1 ? 's' : ''}</span>}
          </div>
        </div>
        ...
        <div className="bg-white border border-gray-200 rounded-xl p-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            <div className="text-center">
              <p className="text-lg font-bold text-green-600">${app.combinedMonthlyIncome.toLocaleString()}</p>
              <p className="text-xs text-gray-500">Combined/mo</p>
            </div>
            <div className="text-center">
              <p className="text-lg font-bold">${(app.rentRequired * 3).toLocaleString()}</p>
              <p className="text-xs text-gray-500">3× Required</p>
            </div>
            <div className="text-center">
              <span className={`px-2 py-1 rounded-full text-xs font-semibold ${
                app.meetsRequirement ? 'text-green-700 bg-green-100' : 'text-red-700 bg-red-100'
              }`}>
            ...
            <div key={i} className="bg-white border border-gray-200 rounded-xl p-4">
              <div className="flex items-start justify-between mb-2">
                <div>
                  ...
                  <p className="text-xs text-gray-500">{m.email} · {m.university}</p>
                </div>
              ...
              <div className="flex items-center justify-between text-sm mt-2">
                ...
                {m.guarantor ? (
                  <span className="flex items-center space-x-1">
                    ...
                    <span className="text-gray-600 text-xs">{m.guarantor.name}:</span>
                    {guarStatus(m.guarantor.verificationStatus)}
                  </span>
                ) : (
                  <span className="text-green-600 text-xs font-medium">No guarantor needed</span>
                )}
              </div>
            </div>
          ))}
        ...
        {app.status === 'applicants_approved' && (
          <div className="space-y-3">
            <div className="bg-green-50 border border-green-200 rounded-xl p-3 text-center">
              ...
              <span className="text-sm font-semibold text-green-800">Applicants approved!</span>
              <p className="text-xs text-green-700 mt-1">
                Open the group chat to communicate and send the lease when ready.
              </p>
            </div>
            <button
        ...
        {app.status === 'fully_executed' && (
          <div className="bg-green-50 border border-green-200 rounded-xl p-4 text-center">
            ...
            <p className="text-xs text-green-700 mt-1">All parties have signed. Property is now LEASED.</p>
          </div>
        )}

        {!alreadyApproved && !readyToApprove && app.status !== 'pending_verifications' && (
          <p className="text-center text-xs text-gray-500">
            {!allComplete && 'Waiting for all applications to complete. '}
            {!allVerified && 'Some guarantors have not yet verified. '}
            {!app.meetsRequirement && 'Combined income does not meet the requirement.'}
          </p>
        )}
      </div>
    );
    ...
    <div className="p-4 pb-20 space-y-4">
      <div className="bg-purple-50 border border-purple-200 rounded-xl p-4">
        ...
        <p className="text-sm text-purple-700">
          Track verifications, review combined income &amp; guarantors, then approve and send a joint lease.
        </p>
      </div>
        ...
        return (
          ...
          >
              ...
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500">
                    {app.propertyTitle} · {app.members.length} tenant{app.members.length !== 1 ? 's' : ''}
                    {totalGuarantors > 0 && `, ${totalGuarantors} guarantor${totalGuarantors !== 1 ? 's' : ''}`}
                  </p>
                </div>
              </div>
              {stageBadge(app.status)}
            ...
            <div className="flex items-center justify-between">
              ...
              <span className={`text-xs px-2 py-1 rounded-full ${
                app.meetsRequirement ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'
              }`}>
...
const LandlordInbox = ({ properties, onSelectApplicant, onSendMessage, onScheduleTour, onSendLease, onNavigateToApprovals, onNavigateToUtilities, onApproveGroupApplication, onNavigateToMultiPartyLease, onOpenWaitingRoom, onOpenGroupChat }) => {
  ...
  const getTourStatusEmoji = (tourStatus) => {
    switch (tourStatus) {
      case 'not-requested': return '❓'; // No tour requested
      case 'requested': return '📅'; // Tour requested, not scheduled
      case 'scheduled': return '✅'; // Tour scheduled
      case 'completed': return '✔️'; // Tour completed
      default: return '❓';
    }
  };
  ...
  const getTourStatusText = (tourStatus) => {
    switch (tourStatus) {
      case 'not-requested': return 'No Tour Requested';
      case 'requested': return 'Tour Requested';
      case 'scheduled': return 'Tour Scheduled';
      case 'completed': return 'Tour Completed';
      default: return 'No Tour Info';
    }
  };
      ...
      <div className="pb-20">
        ...
        <div className="flex bg-gray-100 rounded-lg p-1 mb-4">
          ...
          <button
            ...
            className={`flex-1 py-2 rounded-md text-sm font-medium transition-colors flex items-center justify-center ${
              activeTab === 'group' ? 'bg-white text-purple-600 shadow-sm' : 'text-gray-600'
            }`}
          >
        ...
        {activeTab === 'group' && (
          <GroupApplicationsTab
            ...
            onNavigateToMultiPartyLease={app => onNavigateToMultiPartyLease?.(app)}
            onOpenWaitingRoom={app => onOpenWaitingRoom?.(app)}
            onOpenChat={app => onOpenGroupChat?.(app)}
          />
        )}

        {activeTab === 'individual' && <div className="p-4">
            ...
            {applications.map(application => (
              ...
              >
                ...
                <div className="flex items-center mb-3">
                  <div className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${getTourStatusColor(application.tourStatus)}`}>
                    <span className="mr-1">{getTourStatusEmoji(application.tourStatus)}</span>
                    {getTourStatusText(application.tourStatus)}
                    {application.tourDate && application.tourStatus === 'scheduled' && (
                      <span className="ml-2 text-xs">
                        • {new Date(application.tourDate).toLocaleDateString()} at {new Date(application.tourDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                      </span>
                    )}
                  </div>
                </div>
    ...
    return (
      <div className="p-4 pb-20">
        ...
        <div className="bg-white border border-gray-200 rounded-lg p-6 mb-6">
          ...
          {/* Tour Status */}
          <div className={`flex items-center justify-between p-3 rounded-lg ${getTourStatusColor(selectedApplicant.tourStatus)}`}>
            <div className="flex items-center">
              <span className="text-2xl mr-2">{getTourStatusEmoji(selectedApplicant.tourStatus)}</span>
              <div>
                <span className="text-sm font-medium">{getTourStatusText(selectedApplicant.tourStatus)}</span>
                {selectedApplicant.tourDate && (
                  <p className="text-xs mt-1">
                    {selectedApplicant.tourStatus === 'scheduled' && 'Scheduled for: '}
                    {selectedApplicant.tourStatus === 'completed' && 'Completed on: '}
                    {new Date(selectedApplicant.tourDate).toLocaleDateString()} at {new Date(selectedApplicant.tourDate).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                  </p>
                )}
              </div>
            </div>
