...
export const calculateCompatibilityScore = (seeker, candidate) => {
  ...
  const budgetScore = calculateBudgetCompatibility(seeker.budget, candidate.budget);
  ...
  const lifestyleWeight = 0.20;
  const lifestyleScore = calculateLifestyleCompatibility(seeker.lifestyle, candidate.lifestyle);
  ...
  const personalWeight = 0.10;
  const personalScore = calculatePersonalCompatibility(seeker.preferences, candidate.preferences);
  totalScore += personalScore * personalWeight;
  totalWeight += personalWeight;

  // Demographics compatibility (weight: 10%)
  const demoWeight = 0.10;
...
const calculateLocationCompatibility = (seeker, candidate) => {
  ...
  if (seeker.university === candidate.university) {
    score += 0.4;
  } else if (seeker.preferredUniversities?.includes(candidate.university) || 
  ...
  if (commonNeighborhoods?.length > 0) {
    score += 0.3 * (commonNeighborhoods.length / Math.max(
...
const calculateLifestyleCompatibility = (seekerLifestyle, candidateLifestyle) => {
  let score = 0;
  let factors = 0;
  
  const lifestyleFactors = [
    'cleanliness', 'noiseLevels', 'socialPreference', 'studyHabits', 
    'sleepSchedule', 'guestPolicy', 'petFriendly', 'smoking'
  ];
  ...
  lifestyleFactors.forEach(factor => {
    if (seekerLifestyle[factor] && candidateLifestyle[factor]) {
      ...
      if (seekerLifestyle[factor] === candidateLifestyle[factor]) {
        score += 1;
      } else if (isCompatibleLifestyleFactor(factor, seekerLifestyle[factor], candidateLifestyle[factor])) {
        score += 0.5;
      }
    }
  });
...
const isCompatibleLifestyleFactor = (factor, value1, value2) => {
  const compatibilityMap = {
    cleanliness: { 'average': ['neat', 'relaxed'], 'neat': ['average'], 'relaxed': ['average'] },
    noiseLevels: { 'moderate': ['quiet', 'lively'], 'quiet': ['moderate'], 'lively': ['moderate'] },
    socialPreference: { 'balanced': ['social', 'independent'], 'social': ['balanced'], 'independent': ['balanced'] },
    studyHabits: { 'flexible': ['morning', 'evening', 'night'], 'morning': ['flexible'], 'evening': ['flexible'], 'night': ['flexible'] }
  };
...
const calculateAcademicCompatibility = (seeker, candidate) => {
  ...
  if (seeker.academicLevel === candidate.academicLevel) {
    score += 0.3;
  } else if (Math.abs(getAcademicLevelNumber(seeker.academicLevel) - getAcademicLevelNumber(candidate.academicLevel)) <= 1) {
    score += 0.15;
  }
...
const getAcademicLevelNumber = (level) => {
  const levels = { 'freshman': 1, 'sophomore': 2, 'junior': 3, 'senior': 4, 'graduate': 5, 'phd': 6 };
...
const areSimilarMajors = (major1, major2) => {
  ...
  const businessMajors = ['business', 'economics', 'finance', 'marketing', 'accounting'];
  const artsMajors = ['english', 'history', 'art', 'music', 'theater', 'philosophy'];
  const socialMajors = ['psychology', 'sociology', 'political-science', 'anthropology'];
  
  const majorGroups = [stemMajors, businessMajors, artsMajors, socialMajors];
  
  return majorGroups.some(group => group.includes(major1) && group.includes(major2));
};
...
const calculatePersonalCompatibility = (seekerPrefs, candidatePrefs) => {
  ...
  if (seekerPrefs.hobbies && candidatePrefs.hobbies) {
    const commonHobbies = seekerPrefs.hobbies.filter(h => candidatePrefs.hobbies.includes(h));
    const totalHobbies = Math.max(seekerPrefs.hobbies.length, candidatePrefs.hobbies.length);
  ...
  if (seekerPrefs.personality && candidatePrefs.personality) {
    const personalityScore = calculatePersonalityCompatibility(seekerPrefs.personality, candidatePrefs.personality);
...
const calculateDemographicCompatibility = (seeker, candidate) => {
  ...
  if (seeker.genderPreference) {
    if (seeker.genderPreference === 'any' || seeker.genderPreference === candidate.gender) {
      score += 0.5;
    }
  } else {
...
export const generateMatchExplanation = (seeker, candidate, score) => {
  ...
  if (seeker.preferences?.hobbies && candidate.preferences?.hobbies) {
    ...
    if (commonHobbies.length > 0) {
      explanations.push(`🎯 Shared interests: ${commonHobbies.slice(0, 2).join(', ')}`);
    }
  }
  ...
  const matches = candidates
    .map(candidate => {
      ...
      if (includeExplanations) {
        match.matchExplanation = generateMatchExplanation(seeker, candidate, score);
      }
