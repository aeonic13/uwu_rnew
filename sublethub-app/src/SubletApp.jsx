...
import SubleaseContractManager from './SubleaseContractManager';
...
const SubletApp = () => {
  ...
  const handleSignOut = () => {
    setUser(null);
    setShowLogin(true);
    setCurrentView('browse'); // Will be handled by login screen
    setFavorites([]);
    setMessages([]);
    setTransactions([]);
    setTourRequests([]);
  ...
  if (showLogin) {
    return <EnhancedRegistration onComplete={(userData) => {
      ...
      // Set appropriate initial view based on user type and intent
      if (userData.userType === 'owner') {
        setCurrentView('owner-dashboard'); // Property owners start with dashboard
      } else if (userData.userType === 'student') {
        if (userData.intent === 'rent') {
          setCurrentView('browse'); // Students looking to rent start with browse
        } else if (userData.intent === 'sublease') {
          setCurrentView('post'); // Students looking to sublease start with post
        } else {
          setCurrentView('browse'); // Default to browse for students
        }
      } else {
        setCurrentView('browse'); // Default view
      }
    }} />;
  }
  ...
  return (
    <div className="max-w-md mx-auto bg-white min-h-screen">
      ...
      <div className="bg-blue-600 text-white p-4">
        <div className="flex items-center justify-between">
            ...
            <button 
              onClick={() => {
                ...
                if (user?.userType === 'owner') {
                  ...
                  // Student navigation
                  if (['roommates', 'roommate-discovery', 'group-creator', 'add-to-conversation'].includes(currentView)) {
                    setCurrentView('roommates');
                           ...
                           'bank-accounts', 'tenant-rights', 'parent-guardian', 'sublease-contract',
                           'dispute-resolution', 'deposit-protection'].includes(currentView)) {
          ...
          <h1 className="text-xl font-bold flex-1 text-center">
            ...
            {currentView === 'sublease-contract' && 'Sublease Contracts'}
            {currentView === 'owner-approval' && 'Sublease Approvals'}
        ...
        <EnhancedPropertyDetails 
          listing={{
            ...
            owner: {
              ...
              bio: "Graduate student subletting my apartment while studying abroad. I understand student needs and want to make this process smooth for everyone!",
            ...
            >
              <div className="flex items-center">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-600">Sublet your room or space (requires owner approval)</p>
                </div>
              </div>
            </button>
          </div>
        ...
        <StudentListingForm 
          ...
          onSubmit={(listingData) => {
            ...
            alert(
              ...
              '2. Consider creating a sublease contract to protect yourself\n' +
      ...
      {/* Sublease Contract Manager */}
      {currentView === 'sublease-contract' && user?.userType === 'student' && (
        <SubleaseContractManager 
          user={user}
          currentLease={currentLease}
          onBack={() => setCurrentView('profile')}
          onNavigate={(view) => setCurrentView(view)}
      ...
      {!['browse', 'university-search', 'roommates', 'roommate-discovery', 'group-creator', 'add-to-conversation', 'detail', 'post', 'landlord-listing', 'student-listing', 'messages', 'profile', 'application', 'agreement', 'confirmation', 'payments', 'move-in', 'landlord-inbox', 'utility-manager', 'owner-approvals', 'owner-dashboard', 'banking-bookkeeping', 'rent-collection', 'tax-center', 'parent-guardian', 'sublease-contract', 'owner-approval', 'dispute-resolution', 'deposit-protection', 'document-manager', 'property-management', 'security-deposit', 'tenant-rights', 'property-inspection', 'edit-profile', 'my-listings', 'saved-properties', 'application-history', 'notifications', 'privacy-settings', 'help-center', 'contact-support', 'report-maintenance', 'view-lease', 'utilities-management', 'renters-insurance', 'bank-accounts', 'map'].includes(currentView) && (
        <div className="p-4 pb-20 text-center">
          <div className="mt-20">
          ...
          {user?.userType === 'owner' ? (
            ...
            <>
              ...
              <button
                ...
                className={`flex flex-col items-center py-2 px-3 ${
                  ['roommates', 'roommate-discovery', 'group-creator', 'add-to-conversation'].includes(currentView) ? 'text-blue-600' : 'text-gray-400'
                }`}
              >
                <Users size={24} />
