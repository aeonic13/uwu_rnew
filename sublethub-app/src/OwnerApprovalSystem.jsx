...
const OwnerApprovalSystem = ({ user, onBack, onNavigate }) => {
    ...
    pendingRequests: [
      {
        ...
        documents: [
          ...
          { type: 'sublease_agreement', status: 'draft', uploadedDate: '2024-03-24' }
        ],
        riskAssessment: {
          overallRisk: 'low',
          factors: {
  ...
  return (
    <div className="p-4 pb-20">
      <div className="mb-6">
        <h2 className="text-2xl font-bold mb-2">Sublease Approval Center</h2>
        <div className="flex items-center text-gray-600">
          <Shield size={16} className="mr-2" />
          <span>Review and approve sublease requests from your tenants</span>
        </div>
      </div>
            ...
            <div key={request.id} className="bg-white border border-gray-200 rounded-lg p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="text-right">
                  <div className="text-lg font-semibold text-green-600">${request.sublease.requestedRent}/month</div>
                  <div className="text-xs text-gray-500">
                    {request.sublease.duration} months
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
                <div>
                  ...
                  <span className="ml-2 font-medium">{request.sublease.reason}</span>
                </div>
                <div>
                  <span className="text-gray-600">Type:</span>
                  <span className="ml-2 font-medium capitalize">{request.sublease.type}</span>
                </div>
                <div>
                  <span className="text-gray-600">Duration:</span>
                  <span className="ml-2 font-medium">
                    {new Date(request.sublease.startDate).toLocaleDateString()} - {new Date(request.sublease.endDate).toLocaleDateString()}
                  </span>
                </div>
                <div>
          ...
          {approvalData.approvedRequests.map((request) => (
            <div key={request.id} className="bg-white border border-gray-200 rounded-lg p-6">
              <div className="flex items-start justify-between mb-4">
                ...
                <div className="text-right">
                  <div className="text-lg font-semibold text-green-600">${request.sublease.approvedRent}/month</div>
                  <div className="text-xs text-gray-500">
                    {new Date(request.sublease.startDate).toLocaleDateString()} - {new Date(request.sublease.endDate).toLocaleDateString()}
                  </div>
                </div>
          ...
          <div className="bg-white rounded-lg max-w-4xl w-full max-h-[90vh] overflow-y-auto">
            <div className="p-6">
              <div className="flex justify-between items-center mb-6">
                <h2 className="text-xl font-semibold">Sublease Request Details</h2>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className="bg-gray-50 p-4 rounded-lg">
                  ...
                  <div className="space-y-2 text-sm">
                    ...
                    <div><strong>Type:</strong> {selectedRequest.sublease.type}</div>
                    <div><strong>Requested Rent:</strong> ${selectedRequest.sublease.requestedRent}/month</div>
                    <div><strong>Current Rent:</strong> ${selectedRequest.property.currentRent}/month</div>
                    <div><strong>Start Date:</strong> {new Date(selectedRequest.sublease.startDate).toLocaleDateString()}</div>
                    <div><strong>End Date:</strong> {new Date(selectedRequest.sublease.endDate).toLocaleDateString()}</div>
                    <div><strong>Duration:</strong> {selectedRequest.sublease.duration} months</div>
                    <div><strong>Reason:</strong> {selectedRequest.sublease.reason}</div>
                  </div>
                </div>
            ...
            <div className="p-6">
              <div className="flex justify-between items-center mb-4">
                <h2 className="text-xl font-semibold">
                  {approvalDecision === 'approved' ? 'Approve' : 'Reject'} Sublease Request
                </h2>
